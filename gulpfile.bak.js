...
gulp.task("sass", function () {
    ...
    // return a value as the completion hint return 
...
gulp.task("css", ["sass"], function () { // 标注一个依赖，依赖的任务必须在这个任务开始之前被完成 
...
gulp.task("jc", function () {
    ...
    // 监听 js 
    var watcher = gulp.watch("src/@(js|css)/**/*.@(js|scss)", ["js", "css"]);
    // 监视那些文件的变动，以及变动之后执行的任务 
    watcher.on("change", function (event) {
        // 在 CLI 中输出一些提示信息，帮助我们了解程序发生了什么 
        // console.log('File ' + event.path + ' was ' + event.type + ', running tasks...'); 
...
// 实时重载 Domain server 
// gulp.task('serve', function() { // browserSync({ // // 注意：请勿同时设置 proxy 和 server,否则会报错 // proxy: "dev01.com.dev", // // port: 3000, // // browser: ["google chrome", "firefox"], //默认值为"google chrome" // reloadOnRestart: false // }); // // gulp.watch(['*.html', 'dist/css/**/*.css', 'dist/js/**/*js','dist/images/**/*.*', 'views/**/*.php']).on("change", reload); // }); 
...
// gulp.task('serve',function() { // browserSync({ // server: { // // host:'localhost:80', // baseDir: '' // }, // startPath: 'index.php?homepage&a=login' // }); // // gulp.watch(['*.html','css/**/*.css','js/**/*js','images/**/*.*','views/**/*.php'], {cwd: './'}, reload); // }); // 定义默认任务,执行`gulp`会自动执行（个人认为可以把需要频繁执行的任务放在这里，例如：css、js；而图片压缩之类的就不必了） // gulp.task('default', function() { // // // 监听 sass // var watcher = gulp.watch('src/css/sass/**/*.scss', ['sass']); // watcher.on('change', function(event) { // // // 在 CLI 中输出一些提示信息，帮助我们了解程序发生了什么 // // console.log('File ' + event.path + ' was ' + event.type + ', running tasks...'); // console.log('事件路径： ' + event.path + ' 事件类型： ' + event.type + ', 正在执行的任务：style'); // }); // });